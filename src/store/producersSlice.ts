...
export const updateProducer = createAsyncThunk(
	...
	async ({
		...
		const updated = await producersService.updateProducer(
			id,
			payload as Partial<Farmer>
		);
		return updated;
	}
);
...
const producersSlice = createSlice({
	...
	extraReducers(builder) {
		...
		// IDs are strings (persisted in db.json); compare as strings
		builder
			...
			.addCase(createProducer.fulfilled, (state, action) => {
				state.status = 'succeeded';
				// API returns `id` as string; push directly
		...
		builder
			...
			.addCase(updateProducer.fulfilled, (state, action) => {
				state.status = 'succeeded';
				// replace updated producer (ids are strings)
